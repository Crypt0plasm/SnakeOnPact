...
;; =====================
  ...
  (defcap CREDIT_EXISTING (receiver:string receiver-existing-guard:guard)
    (let
      (
        (receiver-read-guard:guard (at "Guard" (read PlebeicSnakeLedger receiver)))
      )
      (enforce (= receiver-existing-guard receiver-read-guard) "Account guards do not match!")
      (compose-capability (IZSNAKEORNOT receiver receiver-existing-guard true))
      (compose-capability (DIRECT_CREDIT receiver)
    )
  )

  (defcap CREDIT_ANEW (receiver:string receiver-future-guard:guard)
    (compose-capability (IZSNAKEORNOT receiver receiver-future-guard false))
    (compose-capability (DIRECT_CREDIT receiver)
  )

  (defcap ABSOLUTE_CREDIT (receiver:string guard:guard switch:bool)
    @doc "Master Credit Capability, which switched between \
    \ CREDIT_EXISTING and CREDIT_ANEW via the boolean switch \
    \ <true> Switch uses CREDIT_EXISTING \
    \ <false> Switch uses CREDIT_ANEW "

    (if (= switch true)
      (compose-capability (CREDIT_EXISTING receiver guard))
      (compose-capability (CREDIT_ANEW receiver guard))
    )
...
;;
  ...
  (defun DirectGuardedBaronicCredit (account:string amount:decimal guard:guard type:integer)
    (EnforceAccountGuard account guard)
  ...
  (defun DirectBaronicCredit (account:string amount:decimal type:integer)
    ...
    (ValidateAccount account)
    (enforce (> amount 0.0) "Direct Baronic Credit amount must be positive!")
    (EnforceSnakePrecision amount)

    (let*
      (
        (typelist: (enumerate 1 16))
        (iz-baronic-type-correct:bool (contains type typelist))
      )
      (enforce (= iz-baronic-type-correct true) "Type is not correctly set!")

    )
  )

  (defun DirectGuardedPlebeicCredit (account:string amount:decimal guard:guard type:integer)
    (EnforceAccountGuard account guard)
  ...
  (defun DirectPlebeicCredit (account:string amount:decimal type:integer)
    ...
    (ValidateAccount account)
    (enforce (> amount 0.0) "Direct Plebeic Credit amount must be positive!")
    (EnforceSnakePrecision amount)

    (let*
      (
        (typelist: [1 2 3 11 14 15])
        (iz-plebeic-type-correct:bool (contains type typelist))
      )
      (enforce (= iz-plebeic-type-correct true) "Type is not correctly set!")

    )
  )

  (defun EnforceAccountGuard (account:string guard:guard)
    @doc "Types are: \
    \ Type 1 = Ouroboros; Type 2 = Auryn; Type 3 = Elite-Auryn \
    \ Type 4 = Vested Ouroboros; Type 5 = Vested Auryn; Type 6 = Vested Elite-Auryn \
    \ Type 7 = Burned Ouroboros; Type 8 = BOVA; Type 9 = Recycled Auryn; Type 10 Recycled Elite-Auryn \
    \ Type 11 = Vesta; Type 12 = Sleeping Vesta; Type 13 = Frozen Vesta \
    \ Type 14 = Gold; Type 15 = Unity; Type 16 = Equity"


    (require-capability (IZSNAKEORNOT account guard true))
    (with-read PlebeicSnakeLedger account
      { "Guard" := g }
      (enforce (= g guard) "Account guards do not match!")
    )
  ...
  (defun CreditOuroborosExisting (account:string amount:decimal guard:guard)
    (require-capability (CREDIT_EXISTING account guard))
    (DirectGuardedPlebeicCredit account amount guard 1)
    (DirectBaronicCredit account amount 1)
  )

  (defun CreditOuroborosAnew (account:string amount:decimal receiver-guard:guard)
    (require-capability (CREDIT_ANEW account receiver-guard))
    (DirectGuardedPlebeicCredit account amount guard 1)
    (DirectBaronicCredit account amount 1)
  )

  (defun CreditOuroborosAbsolute (account:string amount:decimal guard:guard switch:bool)
    (require-capability (ABSOLUTE_CREDIT receiver guard switch))
    (if (= switch true)
      (CreditOuroborosExisting account amount guard)
      (CreditOuroborosAnew account amount guard)
    )
  )
  ;;==================
  (defun CreditExisting (account:string amount:decimal guard:guard type:integer)
    (require-capability (CREDIT_EXISTING account guard))
    (DirectGuardedPlebeicCredit account amount guard type)
    (DirectBaronicCredit account amount type)
  )
